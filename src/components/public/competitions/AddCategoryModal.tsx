...
export default function AddCategoryModal({
  ...
  const [selectedCategory, setSelectedCategory] = useState<
    Category | undefined
  >(categories[0]);
  const [startingYear, setStartingYear] = useState<number | null>(
    new Date().getFullYear() - 2,
  );
  const [endingYear, setEndingYear] = useState<number | null>(
  ...
  useEffect(() => {
    ...
    if (mounted) {
      if (edit) {
        setMenWeight(
          isValidArr(edit.menWeights) ? JSON.parse(edit.menWeights) : [],
        );
        setWomenWeight(
          isValidArr(edit.womenWeights) ? JSON.parse(edit.womenWeights) : [],
        );
        setUnisexWeight(
          isValidArr(edit.unisexWeights) ? JSON.parse(edit.unisexWeights) : [],
        );
  ...
  const handleSubmit = () => {
    ...
    if (validateCategory(data)) {
      ...
      if (edit) {
        // update here
        api.competitions
          .updateCategory(formatCategory(data), edit.id)
          .then(res => {
    ...
    <div>
      {edit ? (
        <EditIcon
          ...
          style={{ color: '#c1c1c1', cursor: 'pointer', marginLeft: '.5em' }}
        />
      ) : (
              ...
              <TableRow style={{ backgroundColor: '#3f51b5', color: '#fff' }}>
                ...
                >
                  ...
                  <TextField
                    ...
                    onChange={e => {
                      setStartingYear(parseInt(e.target.value, 10));
                    }}
                  ...
                  <TextField
                    ...
                    type="number"
                    onChange={e => {
                      setEndingYear(parseInt(e.target.value, 10));
                    }}
                    style={{ color: '#fff', maxWidth: '5em', margin: '0 .5em' }}
                  />
                </TableCell>
                <TableCell
                  style={{ color: '#fff', maxWidth: '5em' }}
                  align="right"
                >
                  <TextField
                    id="allowed-over"
                    ...
                    type="number"
                    onChange={e => {
                      setAllowedOver(parseInt(e.target.value, 10) || 0);
                    }}
