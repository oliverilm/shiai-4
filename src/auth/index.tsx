...
import {
  filterCorrectData,
  handleLocalStoragePopulation,
  UserVerifyResult,
} from '../utils';
import {
  Category,
  CategoryInCompetition,
  CategoryInCompetitionPost,
  Competition,
...
const api = {
  competitions: {
    ...
    categories: (slug: string): Promise<RootObject<any>> =>
      getRequest(`${apiConstants.COMPETITION_CATEGORIES}/${slug}`),
    createCategory: (
      data: CategoryInCompetitionPost,
    ): Promise<RootObject<CategoryInCompetition>> =>
      postRequest(
        `${apiConstants.COMPETITION_CATEGORIES}/${data.competition}`,
        data,
      ),
    updateCategory: (
      data: CategoryInCompetitionPost,
      ...
      putRequest(`${apiConstants.COMPETITION_CATEGORY_UPDATE}/${id}`, data),
    deleteCategory: (id: number): Promise<RootObject<any>> =>
      deleteRequest(`${apiConstants.COMPETITION_CATEGORY_DELETE}/${id}`),
  },
  utils: {
    categoryList: (): Promise<RootObject<Category[]>> =>
      getRequest(apiConstants.CATEGORY_LIST),
  },
  club: {
